...
public class AnalyzerVisitor implements ASTVisitor<Void> {
    ...
    public Void visitVarDeclaration(VarDeclarationStatement node) {
        ...
        if (config.activeNamingConvention) {
            if (config.namingConventionFormat.equals(NamingConvention.CAMEL_CASE) && !varName.matches("^[a-z]+([A-Z][a-z0-9]+)*$")) {
                violations.add(new Violation("Variable '" + varName + "' should be in camelCase.", Severity.WARNING, position));
            } else if (config.namingConventionFormat.equals(NamingConvention.SNAKE_CASE) && !varName.matches("^[a-z]+(_[a-z0-9]+)*$")) {
                violations.add(new Violation("Variable '" + varName + "' should be in snake_case.", Severity.WARNING, position));
            }
        }
    ...
    public Void visitCallExpression(CallExpression node) {
        if (config.activeComplexPrintln && node.getCallee().equals("println") && !node.getArguments().isEmpty()) {
            Node argument = node.getArguments().get(0);
            boolean isLiteralOrIdentifier = argument instanceof NumberLiteral
    ...
    @Override
    public Void visitNumberLiteral(NumberLiteral node) { return null; }

    @Override
    public Void visitStringLiteral(StringLiteral node) { return null; }
